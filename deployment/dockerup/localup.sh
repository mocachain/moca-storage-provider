...
function display_help() {
  echo "Usage: $0 [option...] {help|generate|config|reset|start|stop|clean|rebuild|print}" >&2
  echo
  ...
  echo "   config           make sp config.toml"
...
function start_sp() {
  ...
  for sp_dir in "${workspace}/${SP_DEPLOY_DIR}"/*; do
    ...
    if [ ! -f "./${bin_name}${index}" ]; then
      echo "ERROR: Binary file ./${bin_name}${index} not found in ${sp_dir}"
      ((failed_count++))
      cd - >/dev/null || exit
      index=$(($index + 1))
      continue
  ...
  echo ""
  echo "=== SP Startup Summary ==="
  echo "Successfully started: ${success_count} SP(s)"
  echo "Failed to start: ${failed_count} SP(s)"
  ...
  if [ ${success_count} -gt 0 ] && [ ${failed_count} -eq 0 ]; then
    ...
    echo "WARNING: Some storage providers failed to start"
    return 1
  else
    echo "ERROR: All storage providers failed to start"
    return 1
  fi
}
...
function stop_sp() {
  local pids
  local count
  pids=$(LC_ALL=C pgrep -f ${bin_name})
  if [ -n "$pids" ]; then
