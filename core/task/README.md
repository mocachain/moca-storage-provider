...
moca will verify the signature of the approval message to determine whether the SP accepts the bucket and object.
...
successfully sealed on the moca, ensuring a return of the secondary SP. SealObjectTask seals the object on Moca,
...
which collects piece store space by deleting payload data that has been deleted on the moca, GCZombiePieceTask,
which collects piece store space by deleting zombie piece data that resulted from any exception where the piece data 
meta is not on Moca chain, and GCMetaTask, which collects the SP meta store space by deleting expired data.
