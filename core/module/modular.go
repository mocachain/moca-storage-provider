...
import (
	...
	"github.com/mocachain/moca-storage-provider/core/lifecycle"
	"github.com/mocachain/moca-storage-provider/core/rcmgr"
	"github.com/mocachain/moca-storage-provider/core/spdb"
	"github.com/mocachain/moca-storage-provider/core/task"
...
const (
	...
	// AuthOpTypeAgentUpdateObject  defines the agent UpdateObject operator
...
type Manager interface {
	...
	// HandleSealObjectTask handles the result of sealing object to the moca the request comes from TaskExecutor.
	...
	// TriggerRecoverForSuccessorSP is used to trigger recover process
	TriggerRecoverForSuccessorSP(ctx context.Context, vgfID, gvgID uint32, redundancyIndex int32) error
	// QueryRecoverProcess is used to get recover process
...
// Signer is an abstract interface to handle the signature of SP and on moca chain operator.
...
type Signer interface {
	...
	// SwapOut signs the MsgSwapOut and broadcast the tx to moca.
	SwapOut(ctx context.Context, swapOut *virtualgrouptypes.MsgSwapOut) (string, error)
	// SwapOutEvm signs the MsgSwapOut and broadcast the tx to moca by evm tx.
	...
	// SignPeerApprovalRequest signs one of the fixed peer approval request shapes.
	SignPeerApprovalRequest(ctx context.Context, request *gfspserver.GfSpSignPeerApprovalRequest) ([]byte, error)
	...
	// DelegateCreateObject  broadcast the tx to moca.
	DelegateCreateObject(ctx context.Context, msg *storagetypes.MsgDelegateCreateObject) (string, error)
	// DelegateCreateObjectEvm  broadcast the tx to moca by evm tx.
