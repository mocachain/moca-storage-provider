...
`GNFD1-ECDSA` require users to use `private key` for authentication. This mode is used in `moca-go-sdk`. We recommend users using this mode when calling Moca SP RESTful APIs.
...
For `GNFD1-ECDSA` auth type, Moca SP RESTful APIs use `ECDSA-secp256k1` to sign `SignedMsg` field to get `Signature` field. Users can refer the following library to generate `Signature` field:

- [secp256k1](https://github.com/cosmos/cosmos-sdk/tree/main/crypto/keys/secp256k1)

For `GNFD2-EDDSA` auth type, Moca SP RESTful APIs use `Edwards-curve Digital Signature Algorithm` to sign `SignedMsg` field to get `Signature` field.
The EdDSA elliptic curve is Ed25519. 
Users can refer the following library to generate `Signature` field:

- [moca go sdk] https://github.com/mocachain/moca-go-sdk/blob/a21d3b5eb75a211266105ea78ad1c76fcdc87c4d/client/api_off_chain_auth.go#L37-L41
- [moca js sdk] https://github.com/mocachain/moca-js-sdk/blob/9d4463b2b0d845c56ce2093eab15c15fb2ba4787/packages/js-sdk/src/clients/spclient/auth.ts#L69-L70
...
1. Add a `X-Gnfd-Expiry-Timestamp` header into request, to define the expiry timestamp for the generated signature in the authorization headers.
   ...
   See example code at:
   https://github.com/mocachain/moca-go-sdk/blob/e6b5db6bf98e6b9b6a7a20be39d6342381a9ccd4/client/api_client.go#L426-L430

2. Create a canonical request
Create a canonical request by concatenating the following strings, separated by newline characters. This helps ensure that the signature that you calculate and the signature that Moca SP calculates can match.
   ...
   See example code at:
   https://github.com/mocachain/moca-common/blob/8bcfd1ccaf6a8ffc3404abc48260d1f4c7f436b2/go/http/gen_sign_str.go#L75-L86
...
The hash of the canonical request is a slice of byte. Second use `Keccak256` algorithm to get Keccak256 hash.
   ...
   See example code at:
   https://github.com/mocachain/moca-common/blob/8bcfd1ccaf6a8ffc3404abc48260d1f4c7f436b2/go/http/gen_sign_str.go#L100-L102
...
### Code examples in Moca Go SDK

The Moca JS/Go SDK  includes source code on GitHub that shows how to sign Moca SP API requests.

- JS SDK
  - [MsgToSign](https://github.com/mocachain/moca-js-sdk/blob/9d4463b2b0d845c56ce2093eab15c15fb2ba4787/packages/js-sdk/src/clients/spclient/auth.ts#L63)
  - [SignRequest](https://github.com/mocachain/moca-js-sdk/blob/9d4463b2b0d845c56ce2093eab15c15fb2ba4787/packages/js-sdk/src/clients/spclient/auth.ts#L69C1-L70C1)

- GO SDK
  - [MsgToSign](https://github.com/mocachain/moca-common/blob/eb2f0efea22882dee610bd3b06589ed0e50fb8ce/go/http/gen_sign_str.go#L91-L94)
  - [SignRequest](https://github.com/mocachain/moca-go-sdk/blob/a21d3b5eb75a211266105ea78ad1c76fcdc87c4d/client/api_client.go#L676-L685)
...
### CreateBucket code examples in Moca Go SDK

- [CreateBucket](https://github.com/mocachain/moca-go-sdk/blob/develop/client/api_bucket.go#LL113)
- [GetCreateBucketApproval](https://github.com/mocachain/moca-go-sdk/blob/develop/client/api_bucket.go#L64)
...
- [CreateObject](https://github.com/mocachain/moca-go-sdk/blob/develop/client/api_object.go#L98)
- [GetCreateObjectApproval](https://github.com/mocachain/moca-go-sdk/blob/develop/client/api_object.go#L526)
...
X-Gnfd-Signed-Msg is a HTTP response header. SP server will add approval signature to data parsed from `X-Gnfd-Unsigned-Msg`. Users should use hex decode to decode it into byte array. Then use protobuf json unmarshal byte array. Finnaly broadcast txn to Moca chain.
...
Moca SP supports both virtual-hosted-style and path-style URI. It's like AWS S3 so you can easily use Moca SP RESTful APIs.

In a virtual-hosted–style URI, the bucket name is part of the domain name in the URL. Moca SP virtual-hosted–style URLs use the following format:

```shell
https://BucketName.devnet-sp*.mocachain.org/key-name
```

In this example, `EXAMPLE-BUCKET` is the bucket name and `sp.pdf` is the key name:

```shell
https://EXAMPLE-BUCKET.devnet-sp*.mocachain.org/sp.pdf
```

## Path-style requests

In Moca SP, path-style URLs use the following format:

```shell
https://devnet-sp*.mocachain.org/bucket-name/key-name
```
...
```shell
https://devnet-sp*.mocachain.org/EXAMPLE-BUCKET/sp.pdf
```
