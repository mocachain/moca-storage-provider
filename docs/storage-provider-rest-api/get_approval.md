...
| ---------------- | --------------------------- |
| Host(path-style) | devnet-sp*.mocachain.org    |
| Path(path-style) | /moca/admin/v1/get-approval |
| Method           | GET                         |
...
X-Gnfd-Unsigned-Msg header consists of [MsgMigrateBucket](#msgmigratebucket). You can read [Moca headers](/README.md) to know how to marshal and unmarshal them.
...
```HTTP
GET /moca/admin/v1/get-approval?action=action HTTP/1.1
...
```HTTP
GET /moca/admin/v1/get-approval?action=MigrateBucket HTTP/1.1
Host: devnet-sp1.mocachain.org
