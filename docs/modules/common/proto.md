...
message GfSpCreateBucketApprovalTask {
  GfSpTask task = 1;
  moca.storage.MsgCreateBucket create_bucket_info = 2;
}
```
...
message GfSpMigrateBucketApprovalTask {
  GfSpTask task = 1;
  moca.storage.MsgMigrateBucket migrate_bucket_info = 2;
}
```
...
message GfSpCreateObjectApprovalTask {
  GfSpTask task = 1;
  moca.storage.MsgCreateObject create_object_info = 2;
}
```
...
message GfSpDelegateCreateObjectApprovalTask {
  GfSpTask task = 1;
  moca.storage.MsgDelegateCreateObject delegate_create_object = 2;
  bytes fingerprint = 3;
}
```
...
message GfSpReplicatePieceApprovalTask {
  GfSpTask task = 1;
  moca.storage.ObjectInfo object_info = 2;
  moca.storage.Params storage_params = 3;
...
message GfSpUploadObjectTask {
  ...
  moca.storage.ObjectInfo object_info = 3;
  moca.storage.Params storage_params = 4;
...
message GfSpDownloadObjectTask {
  GfSpTask task = 1;
  moca.storage.ObjectInfo object_info = 2;
  moca.storage.BucketInfo bucket_info = 3;
  moca.storage.Params storage_params = 4;
  int64 low = 5;
  int64 high = 6;
}
...
message GfSpMigrateGVGTask {
  GfSpTask task = 1;
  uint64 bucket_id = 2;
  moca.virtualgroup.GlobalVirtualGroup src_gvg = 3;
  moca.virtualgroup.GlobalVirtualGroup dest_gvg = 4;
  int32 redundancy_idx = 5;
  moca.sp.StorageProvider src_sp = 6;
...
Some structured data used in GfSp is deinfed in Moca chain repo, we display them as follows.
...
message MsgStorageProviderExit {
  ...
  // storage_provider defines the operator account address of the storage provider who want to exit from the moca storage network.
  string storage_provider = 1 [(cosmos_proto.scalar) = "cosmos.AddressString"];
}
```
