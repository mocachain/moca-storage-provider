...
before serving the bucket and object. If SP approves the message, it will sign the approval message. The moca will
...
the piece. The secondary SP uses this information to confirm whether the object was successfully sealed on moca,
ensuring a return of the secondary SP. SealObjectTask seals the object on Moca, while the DownloadObjectTask allows
...
piece store space by deleting payload data that has been deleted on the moca, `GCZombiePieceTask` which collects piece
store space by deleting zombie piece data that resulted from any exception where the piece data meta is not on Moca
...
Considering the change of storage params on the moca, the storage params of each object should be determined when
...
type ReplicatePieceTask interface {
    ...
    // GetSealed returns an indicator whether successful seal object on moca
...
type ReceivePieceTask interface {
    ...
    // sealed on moca, the secondary SP has an incentive to confirm that otherwise
...
The SealObjectTask is an abstract interface to  record the information for sealing object to the Moca chain.
...
data that the object has been deleted on Moca chain.
