...
It is assumed that each SP provides more stable service and won’t go offline as enough amoca are staking to be one SP in Moca. But as there are a few SPs distributed all over the world in Moca, Moca still needs to have its own redundancy strategy in case some SP goes offline for some unexpected reasons. 
Considering the data locality, people can get better storage service from the SPs in the same region. So Moca provides below redundancy strategy:

- Replica and EC are used in the Moca network; Default EC is 4+2, 4 data chunks and 2 parity chunks.
...
Although each SP may provide a more stable service and won't go offline as enough amoca are staking to be one SP, Moca still establish its own redundancy strategy to get rid of the dependency on a single SP and support the data availability in a decentralized way. Here below is the basic design idea.
