...
Metadata service is to supply better query service for the Moca network. Users can interact with SP for some complex query services.
...
Sync all the Moca chain data to the distributed stores, and offers the read RPC
requests for chain data(in addition to payload). SP service will query the info, E.g.
permission, ListBucket, ListObject, etc. It will reduce the pressure on the Moca chain.
