...
Garbage Collection(GC) is the background service running in SP, which is used to recycle the storage space of those deleted objects on the Moca chain. GC service is running in background periodically. It comprises below steps:
...
TaskExecutor module code implementation: [TaskExecutor](https://github.com/mocachain/moca-storage-provider/tree/master/modular/taskexecutor)
