...
##############################################################################
# pad exported private keys to 64 hex chars                                  #
# mocad strips leading zeros on export, and a 63-char key makes an SP fail   #
# to start with "len of Keybytes is not equal to 32" on unlucky runs         #
##############################################################################
function normalize_sp_private_keys() {
  ...
  jq '
    ...
    with_entries(
      .value |= with_entries(
        if (.key | endswith("PrivateKey")) and (.value | type == "string") then
          .value |= pad64
        else
          .
        end
...
function moca_sp() {
  ...
  make install-tools
  make build
  # the chain registers 8 SPs and the default redundancy needs them all serving
  sed -i -e "s/^SP_NUM=.*/SP_NUM=8/g" ./deployment/localup/env.info
