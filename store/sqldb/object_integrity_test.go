...
func TestSpDBImpl_SetObjectIntegritySuccess(t *testing.T) {
	...
	s, mock := setupDB(t)
	mock.ExpectBegin()
	mock.ExpectExec("INSERT INTO `integrity_meta_00` (`object_id`,`redundancy_index`,`integrity_checksum`,`piece_checksum_list`,`object_size`) VALUES (?,?,?,?,?)").
...
func TestSpDBImpl_ListIntegrityChecksumSuccess(t *testing.T) {
	...
	metas, err := s.ListIntegrityMetaByObjectIDRange(0, 1000, true)
	assert.Equal(t, len(metas), IntegrityMetasNumberOfShards)
	assert.Nil(t, err)
}
...
func TestSpDBImpl_DeleteAllReplicatePieceChecksumOptimizedSuccess(t *testing.T) {
	...
	s, mock := setupDB(t)
	mock.ExpectBegin()
	mock.ExpectExec("DELETE FROM `piece_hash` WHERE object_id = ? and redundancy_index = ?").
		WithArgs(objectID, redundancyIdx).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()
