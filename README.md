...
Moca Storage Provider (abbreviated SP) is storage service infrastructure provider. It uses Moca as the ledger and the single source of truth. Each SP can and will respond to users' requests to write (upload) and read (download) data, and serve as the gatekeeper for user rights and authentications.
...
SPs have to register themselves first by depositing on the Moca blockchain as their "Service Stake". Moca validators will go through a dedicated governance procedure to vote for the SPs of their election. SPs are encouraged to advertise their information and prove to the community their capability, as SPs have to provide a professional storage system with high-quality SLA.
...
When users want to write an object into Moca, they or the client software they use must specify the primary SP. Primary SP should be used as the only SP to download the data. Users can change the primary SP for their objects later if they are not satisfied with its service.
...
# show the moca-sp version information
Moca Storage Provider
    __                                                       _     __
...
PathPattern = [
    ...
    {Key = "/Moca/admin/v1/get-approval", Method = "GET", Names = ["GetApproval"]},
    {Key = "/Moca/admin/v1/challenge", Method = "GET", Names = ["GetChallengeInfo"]},
    {Key = "/Moca/receiver/v1/replicate-piece", Method = "PUT", Names = ["ReplicateObjectPiece"]},
    {Key = "/Moca/recovery/v1/get-piece", Method = "GET", Names = ["RecoveryPiece"]},
    {Key = "/Moca/migrate/v1/notify-migrate-swap-out-task", Method = "POST", Names = ["NotifyMigrateSwapOut"]},
    {Key = "/Moca/migrate/v1/migrate-piece", Method = "GET", Names = ["MigratePiece"]},
    {Key = "/Moca/migrate/v1/migration-bucket-approval", Method = "GET", Names = ["MigrationBucketApproval"]},
...
- `ChainID` of testnet is `moca_5151-1`.
...
- `P2PPrivateKey` and `node_id` is generated by `./moca-sp p2p.create.key -n 1`
...
[Run Testnet SP Node](https://docs.moca.network/)

## Document

- [Moca Whitepaper](https://docs.moca.network/): The official Moca Whitepaper.
- [Moca](https://docs.moca.network/): The Moca documents.
- [Storage Module on Moca](https://docs.moca.network/): The storage module on Moca Chain.
- [Storage Provider on Moca](https://docs.moca.network/): The storage provider on Moca Chain.
- [Data Availability Challenge](https://docs.moca.network/): The correctness of payload be stored in SP.
- [SP Introduction](https://docs.moca.network/): The Moca Storage Provider documents.
- [SP Compiling and Dependencies](https://docs.moca.network/): The detailed introduction to sp compiling and dependencies.
- [Run Local SP Network](https://docs.moca.network/): The introduction to run local SP env for testing.
- [Run Testnet SP Node](https://docs.moca.network/): The introduction to run testnet SP node.

## Related Projects

- [Moca](https://github.com/mocachain/moca): The Golang implementation of the Moca Blockchain.
- [Moca-Go-SDK](https://github.com/mocachain/moca-go-sdk): The Moca SDK, interact with SP, Moca and Tendermint.
- [Moca Cmd](https://github.com/mocachain/moca-cmd): Moca client cmd tool, supporting commands to make requests to Moca.
- [Moca-Common](https://github.com/mocachain/moca-common): The Moca common package.
...
- [Moca-Juno](https://github.com/mocachain/moca-juno): The Cosmos Hub blockchain data aggregator and exporter package.
...
If you'd like to contribute to Moca Storage Provider, please fork, fix, commit and
...
The Moca storage provider library (i.e. all code outside the `cmd` directory) is licensed under the
[GNU Lesser General Public License v3.0](https://www.gnu.org/licenses/lgpl-3.0.en.html),
also included in our repository in the `COPYING.LESSER` file.

The Moca storage provider binaries (i.e. all code inside the `cmd` directory) is licensed under the
