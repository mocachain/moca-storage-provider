...
import (
	...
	localDB "github.com/mocachain/moca-storage-provider/modular/blocksyncer/database"
...
func (i *Impl) Process(height uint64) error {
	...
	finalizeBlockEvents := events.FinalizeBlockEvents
	txCount := len(txs)
	...
	// 2. handle events in Block
	if len(finalizeBlockEvents) > 0 {
		sqls, err = i.ExportEventsWithoutTx(ctx, block, finalizeBlockEvents)
		if err != nil {
