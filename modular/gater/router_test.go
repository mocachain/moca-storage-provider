...
func TestRouters(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			router:           gwRouter,
			method:           http.MethodGet,
			url:              fmt.Sprintf("%s%s/%s?%s", scheme, mockBucketName, testDomain, GetBucketMigrationProgressQuery),
			shouldMatch:      true,
		...
		},
		{
			...
			router:           gwRouter,
			method:           http.MethodPost,
			url:              fmt.Sprintf("%s%s/4pheou4squv/b0q?delegate=&complete=false&offset=0", scheme, testDomain),
			shouldMatch:      true,
