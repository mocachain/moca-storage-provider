...
func TestUserOffChainAuthSignature(t *testing.T) {
	t.Skip("skip: EDDSA key generation test helper needs rework")
...
// GenerateEddsaPrivateKey generates an eddsa private key from a deterministic seed.
func GenerateEddsaPrivateKey(seed string) (sk *PrivateKey, err error) {
	buf := make([]byte, 32)
	copy(buf, seed)
	reader := bytes.NewReader(buf)
	return eddsa.GenerateKey(reader)
}
