...
func (e *ExecuteModular) recoverByPrimarySP(ctx context.Context, task coretask.RecoveryPieceTask) error {
	...
	// compare integrity hash; a successor has no local integrity record and is
	// validated against the sealed object checksums once the piece set completes
...
func (e *ExecuteModular) recoverBySecondarySP(ctx context.Context, task coretask.RecoveryPieceTask, isMyselfSecondary bool) error {
	...
	downLoadPieceSize := 0
	for _, pieceData := range recoveryDataSources {
		if pieceData == nil {
			continue
		}
...
func (e *ExecuteModular) setPieceMetadata(ctx context.Context, task coretask.RecoveryPieceTask, pieceData []byte) error {
	...
	if len(pieceChecksums) == int(segmentCount) {
		...
		// the assembled record must match the checksums sealed on chain before it
		// becomes servable; otherwise drop the accumulated checksums so the failed
		// object scheduler re-recovers every piece from scratch
		chainInfo, queryErr := e.baseApp.Consensus().QueryObjectInfoByID(ctx, task.GetObjectInfo().Id.String())
		if queryErr != nil {
			log.CtxErrorw(ctx, "failed to query object info from chain when recovery", "object_id", objectID, "error", queryErr)
			return queryErr
		}
		checksumIdx := int(redundancyIdx) + 1
		if checksumIdx < 0 || checksumIdx >= len(chainInfo.GetChecksums()) {
			log.CtxErrorw(ctx, "recovery redundancy index out of sealed checksum bounds", "object_id", objectID,
				"redundancy_index", redundancyIdx, "checksum_count", len(chainInfo.GetChecksums()))
			return ErrRecoveryChainChecksum
		}
		if !bytes.Equal(integrityChecksum, chainInfo.GetChecksums()[checksumIdx]) {
			log.CtxErrorw(ctx, "recovered data does not match the sealed object checksums", "object_id", objectID,
				"redundancy_index", redundancyIdx, "expected", hex.EncodeToString(chainInfo.GetChecksums()[checksumIdx]),
				"actual", hex.EncodeToString(integrityChecksum))
			if delErr := e.baseApp.GfSpDB().DeleteAllReplicatePieceChecksum(objectID, redundancyIdx, segmentCount); delErr != nil {
				log.CtxErrorw(ctx, "failed to delete replicate piece checksums after chain mismatch", "object_id", objectID, "error", delErr)
			}
			return ErrRecoveryChainChecksum
		}
