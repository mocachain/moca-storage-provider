...
var spOperatorAddressFlag = &cli.StringFlag{
	...
	Usage:    "The operator account address of the storage provider who want to exit from the moca storage network",
	Required: true,
}

var SPExitCmd = &cli.Command{
	Name:  "sp.exit",
	Usage: "Used for sp exits from the Moca storage network",
	Description: `Using this command, it will send an transaction to Moca blockchain to tell this SP is prepared ` +
		`to exit from Moca storage network.`,
...
var CompleteSPExitCmd = &cli.Command{
	...
	Usage: "Only used in debugging scenarios, online use not allowed. Used for sp complete exits from the Moca storage network.",
	Description: `Using this command, it will send an transaction to Moca blockchain to tell this SP is prepared ` +
		`to complete exit from Moca storage network.`,
	Category: migrateCommands,
...
var CompleteSwapOutCmd = &cli.Command{
	...
	Usage: "Only used in debugging scenarios, online use not allowed. Used for swap out from the Moca storage network.",
	Description: `Using this command, it will send an transaction to Moca blockchain to tell this SP is prepared ` +
		`to swap out from Moca storage network.`,
	Category: migrateCommands,
