...
func TestListModules(t *testing.T) {
	...
	err := app.Run([]string{"./moca-sp", "list.modules"})
	assert.Nil(t, err)
}
...
func TestListErrors(t *testing.T) {
	...
	err := app.Run([]string{"./moca-sp", "list.errors"})
	assert.Nil(t, err)
}
...
func TestQueryTasks(t *testing.T) {
	...
	err := app.Run([]string{"./moca-sp", "query.task", "--task.key", "abc", "--config", "not_exist_config"})
	assert.NotNil(t, err)
	...
	err = app.Run([]string{"./moca-sp", "query.task", "--task.key", "abc", "--config", DefaultConfigFile})
	assert.NotNil(t, err)
...
func TestGetObject(t *testing.T) {
	...
	err := app.Run([]string{"./moca-sp", "get.object", "--object.id", "100"})
	assert.Nil(t, err)
}
...
func TestGetChallengePieceInfo(t *testing.T) {
	...
	err := app.Run([]string{"./moca-sp", "challenge.piece", "--object.id", "100", "--redundancy.index", "0", "--segment.index", "0"})
	assert.NotNil(t, err)
}
...
func TestGetSegmentIntegrity(t *testing.T) {
	...
	err := app.Run([]string{"./moca-sp", "get.piece.integrity", "--object.id", "100"})
	assert.Nil(t, err)
}
...
func TestQueryBucketMigrate(t *testing.T) {
	...
	err = app.Run([]string{"./moca-sp", "query.bucket.migrate", "--config", DefaultConfigFile, "--endpoint", "localhost:2012"})
	assert.Nil(t, err)
	// clear temp config file
	os.Remove(DefaultConfigFile)
}
...
func TestQuerySPExit(t *testing.T) {
	...
	err = app.Run([]string{"./moca-sp", "query.sp.exit", "--config", DefaultConfigFile, "--endpoint", "localhost:2012"})
	assert.Nil(t, err)
	// clear temp config file
	os.Remove(DefaultConfigFile)
}
...
func TestGetPrimarySPIncome(t *testing.T) {
	...
	err = app.Run([]string{"./moca-sp", "query.primary.sp.income", "--config", DefaultConfigFile, "--sp.id", "1"})
	assert.Nil(t, err)
	// clear temp config file
	os.Remove(DefaultConfigFile)
}
...
func TestGetSecondarySPIncome(t *testing.T) {
	...
	err = app.Run([]string{"./moca-sp", "query.secondary.sp.income", "--config", DefaultConfigFile, "--sp.id", "1"})
	assert.Nil(t, err)
	// clear temp config file
	os.Remove(DefaultConfigFile)
}
