...
services:
  ...
  init:
    ...
    command: >
      ...
      cp /usr/bin/moca-sp /workspace/build/moca-sp &&
      bash localup.sh --generate /workspace/sp.json root moca mysql:3306 && 
...
{{- range .Nodes }}
  spnode-{{.NodeIndex}}:
    container_name: moca-sp-{{.NodeIndex}}
    depends_on:
      init:
        condition: service_healthy
    image: "{{$.Image}}"
    ...
    command: >
      ./moca-sp{{.NodeIndex}} --config config.toml </dev/null >log.txt 2>&1 &
{{- end }}
volumes:
  db-data:
  local-env:
...
func main() {
	...
	composeConfig := ComposeConfig{
		Nodes:           nodes,
		Image:           "mocachain/moca-storage-provider",
		MySQLImage:      "mysql:8",
		ProjectBasePath: ".",
		BasePorts:       bp,
	}
